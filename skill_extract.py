...
def load_skill_db():
    ...
    return set(skill.lower() for cat in skill_db.values() for skill in cat)
...
def get_resume_details(resume_text):
    ...
    matched = set(token for token in tokens if token in ALL_SKILLS)
    return {"skills": list(matched), "raw_text": clean_text}
...
def compare_resume_with_jd(resume_details, jd_text):
    ...
    matched_skills = [
        skill for i, skill in enumerate(jd_skills)
